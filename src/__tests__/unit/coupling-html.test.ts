/**
 * Unit tests for the Cross-Team Coupling dashboard HTML generator.
...
import { generateCouplingHtml, type CouplingHtmlConfig } from '../../views/webview/coupling-html.js';
 ...
 */
function createMockConfig(overrides?: Partial<CouplingHtmlConfig>): CouplingHtmlConfig {
  const defaultD3Uri: MockUri = {
    toString: () => 'vscode-webview://d3.min.js',
  };
  const defaultStyleUri: MockUri = {
    toString: () => 'vscode-webview://coupling.css',
  };
...
describe('generateCouplingHtml', () => {
  describe('HTML structure', () => {
    it('should generate valid HTML5 document', () => {
      const config = createMockConfig();
      const html = generateCouplingHtml(config);
    ...
    it('should include title', () => {
      ...
      expect(html).toContain('<title>Cross-Team Coupling</title>');
    });
    ...
    it('should include chart header with title', () => {
      ...
      expect(html).toContain('<h1>Cross-Team Coupling</h1>');
    });
  });
  ...
  describe('Resource loading', () => {
    ...
    it('should include stylesheet link', () => {
      ...
      expect(html).toContain('href="vscode-webview://coupling.css"');
    });
  });
  ...
  describe('Filter controls', () => {
    ...
    it('should include min strength filter', () => {
      const config = createMockConfig();
      const html = generateCouplingHtml(config);

      expect(html).toContain('id="minStrengthFilter"');
      expect(html).toContain('type="range"');
      expect(html).toContain('Min Strength');
    });
  ...
  describe('Chart areas', () => {
    ...
    it('should include loading state', () => {
      ...
      expect(html).toContain('Loading team coupling data');
    });
  ...
  describe('Team visibility container', () => {
    it('should include team visibility container', () => {
      const config = createMockConfig();
      const html = generateCouplingHtml(config);

      expect(html).toContain('id="teamVisibilityContainer"');
      expect(html).toContain('Team Visibility');
    });

    it('should include team toggles area', () => {
      const config = createMockConfig();
      const html = generateCouplingHtml(config);

      expect(html).toContain('id="teamToggles"');
      expect(html).toContain('class="team-toggles"');
    });
  });

  describe('Coupling insights', () => {
    it('should include coupling insights container', () => {
      const config = createMockConfig();
      const html = generateCouplingHtml(config);

      expect(html).toContain('id="couplingInsights"');
      expect(html).toContain('Coupling Insights');
    });

    it('should include most coupled pair insight', () => {
      const config = createMockConfig();
      const html = generateCouplingHtml(config);

      expect(html).toContain('id="mostCoupledPair"');
      expect(html).toContain('Most Coupled Pair');
    });

    it('should include island teams insight', () => {
      const config = createMockConfig();
      const html = generateCouplingHtml(config);

      expect(html).toContain('id="islandTeams"');
      expect(html).toContain('Island Teams');
      expect(html).toContain('Teams with no coupling');
    });

    it('should include highest connectors insight', () => {
      const config = createMockConfig();
      const html = generateCouplingHtml(config);

      expect(html).toContain('id="highestConnectors"');
  ...
  describe('Accessibility', () => {
    it('should include aria labels on interactive elements', () => {
      ...
      expect(html).toContain('aria-label="Minimum coupling strength filter"');
  ...
  describe('Legend', () => {
    it('should include legend container', () => {
      ...
      expect(html).toContain('class="chart-legend coupling-legend"');
    });
  });
  ...
  describe('Chart instructions', () => {
    it('should include usage instructions', () => {
      ...
      expect(html).toContain('Hover over arcs');
      expect(html).toContain('Click on a chord');
    });
  });
  ...
  describe('JavaScript functionality', () => {
    ...
    it('should include message handling', () => {
      ...
      expect(html).toContain("case 'couplingData'");
      expect(html).toContain("case 'sharedFilesData'");
      expect(html).toContain("case 'couplingError'");
    });
    ...
    it('should include chord diagram rendering', () => {
      const config = createMockConfig();
      const html = generateCouplingHtml(config);

      expect(html).toContain('function renderChordDiagram');
    });

    it('should include team colors', () => {
      const config = createMockConfig();
      const html = generateCouplingHtml(config);

      expect(html).toContain('TEAM_COLORS');
    ...
    it('should include strength colors', () => {
      const config = createMockConfig();
      const html = generateCouplingHtml(config);

      expect(html).toContain('STRENGTH_COLORS');
      expect(html).toContain('#ef4444'); // high
      expect(html).toContain('#f59e0b'); // medium
    ...
    it('should include tooltip functions', () => {
      ...
      expect(html).toContain('function showArcTooltip');
      expect(html).toContain('function showChordTooltip');
    ...
    it('should include drill-down request functions', () => {
      ...
      expect(html).toContain('function requestTeamDrillDown');
      expect(html).toContain('function requestTeamPairDrillDown');
    });

    it('should include team toggles rendering', () => {
      const config = createMockConfig();
      const html = generateCouplingHtml(config);

      expect(html).toContain('function renderTeamToggles');
    });

    it('should include coupling insights rendering', () => {
      const config = createMockConfig();
      const html = generateCouplingHtml(config);

      expect(html).toContain('function renderCouplingInsights');
    });
