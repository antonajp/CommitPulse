/**
 * Release Management Contributions chart webview panel manager.
 ...
 */
const CLASS_NAME = 'ReleaseMgmtPanel';

/**
 * View type identifier for the release management webview.
 */
const VIEW_TYPE = 'gitrx.releaseMgmtPanel';
...
/**
 * Manages the Release Management Contributions WebviewPanel lifecycle.
 * Implements singleton pattern: only one panel exists at a time.
 * Re-reveals existing panel if user triggers the command again.
 */
export class ReleaseMgmtPanel implements vscode.Disposable {
  /**
   * Singleton panel instance. Null when no panel is open.
   */
  private static currentPanel: ReleaseMgmtPanel | undefined;
  ...
  private dataService: ReleaseManagementDataService | undefined;

  /**
   * Create or reveal the Release Management Contributions panel.
  ...
  static createOrShow(extensionUri: vscode.Uri, secretService: SecretStorageService): void {
    ...
    logger.info(CLASS_NAME, 'createOrShow', 'Opening Release Management Contributions chart panel');

    // If panel exists, reveal it
    if (ReleaseMgmtPanel.currentPanel) {
      logger.debug(CLASS_NAME, 'createOrShow', 'Existing panel found, revealing');
      ReleaseMgmtPanel.currentPanel.panel.reveal(vscode.ViewColumn.One);
      return;
    }
    ...
    if (!ReleaseMgmtPanel.verifyD3Integrity(d3Path, logger)) {
    ...
    // Create a new panel
    logger.debug(CLASS_NAME, 'createOrShow', 'Creating new release management chart webview panel');
    const panel = vscode.window.createWebviewPanel(
      VIEW_TYPE,
      'Gitr: Release Management Contributions',
    ...
    ReleaseMgmtPanel.currentPanel = new ReleaseMgmtPanel(panel, extensionUri, secretService);
  }
  ...
  private constructor(
    ...
    this.logger.debug(CLASS_NAME, 'constructor', 'Initializing ReleaseMgmtPanel');
    ...
    this.panel.webview.onDidReceiveMessage(
      (message: ReleaseMgmtWebviewToHost) => {
    ...
    this.logger.info(CLASS_NAME, 'constructor', 'ReleaseMgmtPanel initialized successfully');
  }
  ...
  private updateWebviewContent(): void {
    this.logger.debug(CLASS_NAME, 'updateWebviewContent', 'Generating release management chart webview HTML');
    ...
    const styleUri = webview.asWebviewUri(
      vscode.Uri.joinPath(this.extensionUri, 'media', 'release-mgmt.css'),
    );
    ...
    webview.html = generateReleaseMgmtHtml({
      nonce,
      d3Uri,
      styleUri,
    ...
    this.logger.debug(CLASS_NAME, 'updateWebviewContent', 'Release management chart webview HTML set');
  }

  /**
   * Handle incoming messages from the webview.
   */
  private async handleMessage(message: ReleaseMgmtWebviewToHost): Promise<void> {
      ...
      switch (message.type) {
        case 'requestReleaseMgmtData': {
          ...
          this.postMessage({
            ...
            environmentDistribution: data.environmentDistribution,
   ...
   * Lazily initializes the DatabaseService and ReleaseManagementDataService.
   */
  private async ensureDbConnection(): Promise<void> {
    ...
    this.logger.debug(CLASS_NAME, 'ensureDbConnection', 'Initializing database connection for release management chart');
    ...
    this.dataService = new ReleaseManagementDataService(this.db);
    this.logger.info(CLASS_NAME, 'ensureDbConnection', 'Database connection established for release management chart');
  }

  /**
   * Post a typed message to the webview.
   */
  private postMessage(message: ReleaseMgmtHostToWebview): void {
   ...
   */
  dispose(): void {
    this.logger.info(CLASS_NAME, 'dispose', 'Disposing ReleaseMgmtPanel');

    ReleaseMgmtPanel.currentPanel = undefined;

    if (this.db) {
      this.logger.debug(CLASS_NAME, 'dispose', 'Shutting down release management chart database connection');
    ...
    this.logger.debug(CLASS_NAME, 'dispose', 'ReleaseMgmtPanel disposed');
  }
