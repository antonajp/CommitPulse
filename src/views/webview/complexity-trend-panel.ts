/**
 * Complexity Trend chart webview panel manager.
...
import { generateComplexityTrendHtml } from './complexity-trend-html.js';
...
import type { ComplexityTrendWebviewToHost, ComplexityTrendHostToWebview } from './complexity-trend-protocol.js';

/**
 * Class name constant for structured logging context.
 */
const CLASS_NAME = 'ComplexityTrendPanel';

/**
 * View type identifier for the complexity trend webview.
 */
const VIEW_TYPE = 'gitrx.complexityTrendPanel';

/**
 * Expected SHA-256 hash of the bundled D3.js v7 library.
 * Used for integrity verification per security patterns.
 */
const D3_EXPECTED_SHA256 = 'f2094bbf6141b359722c4fe454eb6c4b0f0e42cc10cc7af921fc158fceb86539';

/**
 * Manages the Complexity Trend WebviewPanel lifecycle.
 * Implements singleton pattern: only one panel exists at a time.
 * Re-reveals existing panel if user triggers the command again.
 */
export class ComplexityTrendPanel implements vscode.Disposable {
  /**
   * Singleton panel instance. Null when no panel is open.
   */
  private static currentPanel: ComplexityTrendPanel | undefined;
  ...
  private dataService: ComplexityTrendDataService | undefined;

  /**
   * Create or reveal the Complexity Trend panel.
  ...
  static createOrShow(extensionUri: vscode.Uri, secretService: SecretStorageService): void {
    ...
    logger.info(CLASS_NAME, 'createOrShow', 'Opening Complexity Trend chart panel');

    // If panel exists, reveal it
    if (ComplexityTrendPanel.currentPanel) {
      logger.debug(CLASS_NAME, 'createOrShow', 'Existing panel found, revealing');
      ComplexityTrendPanel.currentPanel.panel.reveal(vscode.ViewColumn.One);
      return;
    }

    // Verify D3.js bundle integrity
    const d3Path = vscode.Uri.joinPath(extensionUri, 'media', 'd3.min.js').fsPath;
    if (!ComplexityTrendPanel.verifyD3Integrity(d3Path, logger)) {
    ...
    // Create a new panel
    logger.debug(CLASS_NAME, 'createOrShow', 'Creating new complexity trend webview panel');
    const panel = vscode.window.createWebviewPanel(
      VIEW_TYPE,
      'Gitr: Complexity Trend',
    ...
    ComplexityTrendPanel.currentPanel = new ComplexityTrendPanel(panel, extensionUri, secretService);
  }
  ...
  private constructor(
    ...
    this.logger.debug(CLASS_NAME, 'constructor', 'Initializing ComplexityTrendPanel');
    ...
    this.panel.webview.onDidReceiveMessage(
      (message: ComplexityTrendWebviewToHost) => {
    ...
    this.logger.info(CLASS_NAME, 'constructor', 'ComplexityTrendPanel initialized successfully');
  }
  ...
  private updateWebviewContent(): void {
    this.logger.debug(CLASS_NAME, 'updateWebviewContent', 'Generating complexity trend webview HTML');
    ...
    const styleUri = webview.asWebviewUri(
      vscode.Uri.joinPath(this.extensionUri, 'media', 'complexity-trend.css'),
    );
    ...
    webview.html = generateComplexityTrendHtml({
      nonce,
      d3Uri,
      styleUri,
    ...
    this.logger.debug(CLASS_NAME, 'updateWebviewContent', 'Complexity trend webview HTML set');
  }

  /**
   * Handle incoming messages from the webview.
   * Rate limited to prevent excessive database queries (CWE-770).
   */
  private async handleMessage(message: ComplexityTrendWebviewToHost): Promise<void> {
    this.logger.debug(CLASS_NAME, 'handleMessage', `Handling message: ${message.type}`);

    // Rate limiting check (500ms minimum interval between requests)
    ...
    try {
      ...
      switch (message.type) {
        case 'requestComplexityTrendData': {
          ...
          const chartData = await this.dataService.getChartData(message.filters);
          this.postMessage({
            type: 'complexityTrendData',
            data: chartData.data,
        ...
        case 'requestComplexityTrendFilterOptions': {
          const options = await this.dataService.getFilterOptions();
          this.postMessage({
   ...
   * Lazily initializes the DatabaseService and ComplexityTrendDataService.
   */
  private async ensureDbConnection(): Promise<void> {
    ...
    this.logger.debug(CLASS_NAME, 'ensureDbConnection', 'Initializing database connection for complexity trend chart');
    ...
    this.dataService = new ComplexityTrendDataService(this.db);
    this.logger.info(CLASS_NAME, 'ensureDbConnection', 'Database connection established for complexity trend chart');
  }

  /**
   * Post a typed message to the webview.
   */
  private postMessage(message: ComplexityTrendHostToWebview): void {
   ...
   */
  dispose(): void {
    this.logger.info(CLASS_NAME, 'dispose', 'Disposing ComplexityTrendPanel');

    ComplexityTrendPanel.currentPanel = undefined;

    // Reset rate limiter state
    this.rateLimiter.reset();

    if (this.db) {
      this.logger.debug(CLASS_NAME, 'dispose', 'Shutting down complexity trend database connection');
    ...
    this.logger.debug(CLASS_NAME, 'dispose', 'ComplexityTrendPanel disposed');
  }
