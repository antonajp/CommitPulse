-- Rollback for migration 022: Restore vw_sprint_velocity_vs_loc to Linear-only version
-- Ticket: IQS-937

-- Drop the indexes added by migration 022
DROP INDEX IF EXISTS idx_jira_detail_status_change_date;
DROP INDEX IF EXISTS idx_commit_jira_jira_key;

-- Recreate the original Linear-only view from migration 008
...
COMMENT ON VIEW vw_sprint_velocity_vs_loc IS 'Sprint velocity vs LOC for Linear issues (rollback from 022). Ticket: IQS-888';
